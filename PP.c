...
void CalculaAptidao( Cromossomo *populacao , int tamPopulacao , long int *genes , int qtdGenes )
{
    ...
    for ( int i = 0 ; i < tamPopulacao ; ++i )
    {
        populacao[ i ].aptidaoRoleta = ( float ) (( populacao[ i ].aptidao == 0 ) ? maxAptidao : ( maxAptidao /
                                                                                                   populacao[ i ].aptidao )); //Inversamente proporcional a aptidao
...
int main( int argc , char **argv )
{
    ...
    printf( "- Inicio processamento [ %d:%d:%d ]:\n  - Genes [%i] Populacao [%i] Sobreviventes [%i%%-%i] \n  - Elite [%i%%-%i] Renovacao [%i%%-%i] Mutacao [%i%%-%i] Repeticoes [%i]\n" ,
            data_hora_atual->tm_hour , data_hora_atual->tm_min , data_hora_atual->tm_sec , qtdGenes , tamPopulacao ,
            percSobreviventes , qtdSobreviventes , percElite , qtdElite , percRenovacao , qtdRenovacao , percMutacao ,
            qtdMutacao , repeticoes );
